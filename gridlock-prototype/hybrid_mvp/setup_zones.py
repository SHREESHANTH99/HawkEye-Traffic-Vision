"""
setup_zones.py  —  HawkEye ROI Zone Calibration Utility
=========================================================
Run this ONCE before starting edge_client.py.

Controls:
  Left-click  : Add a point to the ROI polygon (up to 4)
  S           : Save zones_config.json and exit
...
def draw_overlay(base_frame) -> None:
    """Redraw points and lines on a fresh copy of the base frame."""
    disp = base_frame.copy()
    color_point = (0, 255, 255)   # yellow
    color_line  = (0, 165, 255)   # orange
    color_fill  = (0, 255, 0)     # green (closed polygon)
    ...
    # Close the polygon once all 4 points are set
    if len(points) == 4:
...
def save_config() -> None:
    ...
    config = {
        "roi_polygon": points,
        "dominant_direction": "right",   # left | right | up | down
