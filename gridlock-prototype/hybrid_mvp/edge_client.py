"""
edge_client.py  —  HawkEye Traffic Violation Detector  v7.0  (Helmet + Triple-Riding MVP)
=============================================================================================

SCOPE (per MVP requirement):
  - All vehicle classes (car, bus, truck, motorcycle, bicycle) are detected & tracked
    so the scene is fully scanned — but violation checks only run on motorcycle/bicycle
    riders.
  - Seatbelt detection (Moondream-based) has been removed entirely — no Ollama call,
    no votes, no driver-window crop, no seatbelt keyword lists.

Violations Detected:
  GEOMETRY (instant, no AI):
    - TRIPLE_RIDING  : 3+ persons on one motorcycle (bounding box overlap)

  YOLO-VERIFIED:
    - NO_HELMET      : motorcycle/bicycle rider without helmet

v7.0 — HELMET MODEL FIX:
  `keremberke/yolov8s-helmet-detection` does not exist on HuggingFace, which is why
  every crop fell through to Moondream and silently produced 0 confirmed violations.
  Replaced with a verified, working alternative:

    Viddesh1/Bike-Helmet-Detectionv2  (raw.githubusercontent.com, weights/best.pt)
    Classes: Bike_Rider, Helmet, No_Helmet — trained specifically on motorcycle
    riders (not construction hard-hats), reported mAP50 ≈ 0.73-0.82 per class.

  This is downloaded once on first run (auto-cached to ./models/), and loaded with
  the SAME `ultralytics.YOLO()` class already used for the COCO model — no new
  package, no version downgrade. (The other common option, keremberke's HF models,
  needs the unmaintained `ultralyticsplus` pinned to ultralytics==8.0.21, which
  would conflict with your current install, and its mAP@0.5 is only ~0.28.)

  Moondream remains as a last-resort fallback only if the download/load fails.

Efficiency changes vs v5.0:
  - HUD counters are now tracked in-memory (a dict of ints) instead of calling
    Path.glob() on disk every single frame — glob-per-frame was the single biggest
    avoidable cost in the render loop.
  - Removed all seatbelt code paths: crop_driver_window, ask_moondream_seatbelt,
    parse_seatbelt, _BELT_WORN / _BELT_MISSING keyword lists, SEATBELT_* config,
    sb_queue_log / sb_log_lock, seatbelt_votes / votes_lock.
  - Removed unused imports (deque, defaultdict) and the unicode ✗/✓ glyphs in
    cv2.putText, which HERSHEY fonts don't actually render correctly.
  - Motor vehicles (car/bus/truck) are now drawn with a single cheap rectangle
    call for situational awareness — no crop, no encode, no AI queue dispatch.

NOTE ON LICENSING: the helmet weights are pulled from a public, unlicensed
community GitHub repo. Fine for a hackathon/MVP demo — if this ever goes beyond
that, verify the repo's terms or train your own weights on a licensed dataset.
"""
...
HELMET_CONF        = 0.40            # confidence for helmet model
...
# ── COCO class IDs (yolov8s.pt is COCO-pretrained) ──────────────────────────
CLS_PERSON     = 0
CLS_BICYCLE    = 1
CLS_CAR        = 2
CLS_MOTORCYCLE = 3
...
# ── HELMET DETECTION (YOLO-based) ────────────────────────────────────────

def ensure_helmet_weights() -> Path | None:
    """Download the helmet weights once and cache locally; reuse on later runs."""
...
def load_helmet_model(device: str):
    """Load the motorcycle helmet detection YOLO model. Returns (model, no_helmet_ids, helmet_ids)."""
...
def check_helmet_yolo(helmet_model, crop_img: np.ndarray,
    ...
    """
    Run the helmet model on a cropped image. Returns 'NO_HELMET', 'HELMET', or 'SKIP'.
    The model also emits a third class (Bike_Rider) that isn't helmet-related —
    detections are filtered to helmet/no-helmet classes before picking the best one,
    so a confident Bike_Rider box can't be mistaken for a helmet verdict.
    """
    ...
    try:
        ...
        if not results or results[0].boxes is None or len(results[0].boxes) == 0:
            return "NO_HELMET"   # nothing detected at all → assume bare head
        boxes = results[0].boxes
        clses = boxes.cls.cpu().numpy().astype(int)
        confs = boxes.conf.cpu().numpy()
        relevant = [(c, cf) for c, cf in zip(clses, confs) if c in helmet_ids or c in no_helmet_ids]
        if not relevant:
            return "NO_HELMET"   # only irrelevant classes (e.g. Bike_Rider) detected
...
def check_helmet_moondream(img_bytes: bytes) -> str:
    """Fallback path — only reached if the helmet YOLO model failed to load."""
...
def draw_scanned_vehicle(display: np.ndarray, box: list, vid: int, lbl: str) -> None:
    """Cheap draw for cars/buses/trucks — detected & tracked, no violation check."""
    x1, y1, x2, y2 = [int(v) for v in box]
...
def main() -> None:
    ...
    vid_status  : dict           = {}   # vehicle_id → status string
    status_lock : threading.Lock = threading.Lock()

    helmet_queued : set            = set()   # one helmet check per vehicle ID
    helmet_lock   : threading.Lock = threading.Lock()

    box_memory  : dict           = {}   # vid → {"box","lbl","last_frame"}
    memory_lock : threading.Lock = threading.Lock()

    # In-memory HUD counters — replaces per-frame Path.glob() disk scans
    ...
    while True:
        ...
        # ── Frame skip: draw from memory only ──────────────────────────
        ...
        # ── MOTORCYCLES / BICYCLES — full violation checks ────────────────
        for veh in bikes:
            ...
            # TRIPLE_RIDING — instant geometric check
            if count >= 3:
                with confirmed_lock:
            ...
            # NO_HELMET — YOLO check, queued once per vehicle ID
            with helmet_lock:
        ...
        # ── CARS / BUSES / TRUCKS — scanned & tracked only, no AI calls ───
        ...
        # ── Memory boxes for vehicles not seen this frame ─────────────────
        with memory_lock:
            for vid, mem in box_memory.items():
                if vid in seen_ids:
                    continue
